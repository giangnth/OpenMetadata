...
from metadata.ingestion.source.database.hudi.metadata import HudiSource
...
MOCK_DATABASE = Database(
    ...
    name="default",
    fullyQualifiedName="hudi.default",
...
MOCK_DATABASE_SCHEMA = DatabaseSchema(
    ...
    name="default",
    fullyQualifiedName="hudi.default.default",
...
class HudiUnitTest(TestCase):
    """
    Add method validations from Hudi ingestion
    """

    config: OpenMetadataWorkflowConfig = OpenMetadataWorkflowConfig.parse_obj(
        MOCK_HUDI_CONFIG
    )
    hudi: HudiSource = HudiSource.create(
        MOCK_HUDI_CONFIG["source"], config.workflowConfig.openMetadataServerConfig
    )
    spark = hudi.spark
    ...
    def setUpClass(cls) -> None:
        ...
        # Set context
        cls.hudi.context.__dict__["database_service"] = MOCK_DATABASE_SERVICE
        cls.hudi.context.__dict__["database"] = MOCK_DATABASE
        cls.hudi.context.__dict__["database_schema"] = MOCK_DATABASE_SCHEMA
        # We pick up the table comments when getting their name and type, so we
        # store the description in the context
        cls.hudi.context.__dict__["table_description"] = "testing around"
    ...
    def test_get_database_names(self):
        database_names = list(self.hudi.get_database_names())
