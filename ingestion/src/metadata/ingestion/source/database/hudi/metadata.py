...
class HudiSource(DatabaseServiceSource):
    """
    Implements the necessary methods to extract
    Database metadata from Hudi Source
    """
    ...
    def create(cls, config_dict, metadata_config: OpenMetadataConnection):
        ...
        connection: HudiConnection = config.serviceConnection.__root__.config
        if not isinstance(connection, HudiConnection):
            raise InvalidSourceException(
                f"Expected HudiConnection, but got {connection}"
            )
    ...
    def yield_tag(
        ...
        """We don't pick up tags from Hudi"""
